...
class TesterBase:
    ...
    def build_model(self):
        ...
        if os.path.isfile(self.cfg.weight):
            ...
            model = model.half()  # 注意：这里只更改推理没有更改训练，每次新的训练以后都要进行AB测试，后续看是否进行混合精度训练
            self.logger.info(
    ...
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if os.path.isfile(pred_save_path):
                ...
                for i in range(0, len(fragment_list), fragment_batch_size):
                    ...
                    for key in input_dict.keys():
                        if isinstance(input_dict[key], torch.Tensor):
                            ...
                            if input_dict[key].dtype == torch.float32:  # 仅转换浮点类型，只更改推理没有更改训练，每次新的训练以后都要进行AB测试
                                input_dict[key] = input_dict[key].half()  # FP16
                ...
                # 验证文件是否存在且非空，满足条件即在推理时可跳过Syncing和后续的计算，可以避免NCCL超时的报错
                if not os.path.exists(pred_save_path):
                    raise RuntimeError(f"Failed to save {pred_save_path}")
                file_size = os.path.getsize(pred_save_path)
                if file_size == 0:
                    raise RuntimeError(f"File {pred_save_path} is empty")
        ...
        time.sleep(5)  # 暂停5秒，确保所有进程都完成npy文件写入
        logger.info("<<<<<<<<<<<<<<<<< End Evaluation <<<<<<<<<<<<<<<<<")
